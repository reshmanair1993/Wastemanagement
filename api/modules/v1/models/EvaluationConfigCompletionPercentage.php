...
class EvaluationConfigCompletionPercentage extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['lsgi_id','start_value_percent','end_value_percent','performance_point'], 'required'],
...
public function deleteCompletionTime($id)
    ...
    {
        $err  = EvaluationConfigCompletionPercentage::find()->where(['>=','end_value_percent',$this->start_value_percent])->andWhere(['<=','start_value_percent',$this->start_value_percent])->andWhere(['status'=>1])->one();
        if($err)
