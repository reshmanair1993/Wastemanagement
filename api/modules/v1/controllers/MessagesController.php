...
class MessagesController extends ActiveController
{
    ...
    public function actionNotificationCount()
    {
        ...
        $lsgiId = isset($modelAccount->fkCustomer->fkWard->fkLsgi)?$modelAccount->fkCustomer->fkWard->fkLsgi->id:null;
        $wardId = isset($modelAccount->fkCustomer->fkWard)?$modelAccount->fkCustomer->fkWard->id:null;
        $accountAuthority = $modelAccount->fkAccountAuthority;
          if($accountAuthority)
          {
            ...
            if($hksId==0&&$modelAccount->green_action_unit_id)
            {
                if($modelAccount->green_action_unit_id)
                {
                    $hksId = $modelAccount->green_action_unit_id;
                }
            }
        ...
        if($lsgiId)
        {
           $query->andWhere(['lsgi_id'=>$lsgiId]); 
        }
        if($hksId)
        {
           $query->andWhere(['hks_id'=>$hksId]); 
        }
        if($wardId)
        {
           $query->andWhere(['ward_id'=>$wardId]); 
        }
        if($wardId)
        {
           $query->andWhere(['ward_id'=>$wardId]); 
        }
        $query->orWhere(['push_message.account_id'=>$userId]);
                   //   ->andWhere(['or',
                   //     ['lsgi_id'=>$lsgiId],
                   //     ['ward_id'=>$wardId],
                   //     ['hks_id'=>$hksId],
    ...
    {
        $hksId = null;
        $query = PushMessage::find()->where(['status'=>1])->orderby('id DESC');
        if ($account_id)
        {
        ...
        $query->orWhere(['push_message.account_id'=>$account_id])
