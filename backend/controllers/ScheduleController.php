...
$non_residential =0;
        ...
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams,$lsgi,$district,$keyword,$ward,$service,$from,$to,$association,$non_residential);
    ...
    public function actionView($id)
    {
        ...
        $modelCustomer = new ScheduleCustomer;
        $dataProvider = new ActiveDataProvider(
        [
           'query' => ScheduleCustomer::getAllQuery()->andWhere(['schedule_id' => $id,'status' => '1']),
           'pagination' => false,
    ...
    public function actionCreate()
    {
        ...
        if($ward&&$service&&$gt):
           if (isset($post['Schedule']['customer_id'])&&isset($post['Schedule']['type'])&&$post['Schedule']['customer_id']!=null&&$post['Schedule']['type']!=null&&sizeof($post['Schedule']['customer_id']) > 0&&$post['Schedule']['customer_id'][0]>0) {
        ...
        if ($modelSchedule->load(Yii::$app->request->post())) {
            ...
            $modelSchedule->account_id_gt= $gt;
              $modelSchedule->residential_association_id= $association;
            // $modelSchedule->green_action_unit_id= $modelUser->green_action_unit_id;
            // $modelSchedule->lsgi_id= $modelUser->lsgi_id;
...
$scheduleDate = isset($modelSchedule->date)?date('d M Y',strtotime($modelSchedule->date)):null;
$modelPushMessage = new PushMessage;
$modelPushMessage->account_id = $value;
$serviceData = Service::find()->where(['id'=>$service])->andWhere(['status'=>1])->one();
$serviceName = isset($serviceData)?$serviceData->name:'';
$wardData = Ward::find()->where(['status'=>1])->andWhere(['id'=>$ward])->one();
$wardName = isset($wardData)?$wardData->name:'';
$modelPushMessage->message = 'Service scheduled successfully. Service is '. $serviceName. ' Ward : '.$wardName.' Total customers : '.sizeof($post['Schedule']['customer_id']).' Date : '.$scheduleDate;
...
$result = Yii::$app->message->sendMessage($key,$value,$modelPushMessage->message);
      ...
      {//test
        if(isset($post['association'])&&$post['association']!=null){
         $qry = "SELECT DISTINCT customer.created_at,customer.lead_person_name,customer.id as customer_id,account.id as account_id,customer.ward_id as ward_id,customer.building_type_id as building_type_id,account_authority.account_id_customer as account_id_customer,building_type.name as building_type_name,customer.building_number as building_number,customer.address as address, customer.association_name as association_name, customer.association_number as association_number,customer.residential_association_id as residential_association_id FROM customer LEFT JOIN account ON account.customer_id = customer.id 
...
-- LEFT JOIN service_assignment on service_assignment.service_request_id=service_request.id
                     ...
                     and (account.id,account_service.service_id) NOT IN (SELECT account_id_customer,service_request.service_id FROM service_request LEFT JOIN service_assignment on service_assignment.service_request_id=service_request.id where service_assignment.servicing_status_option_id is null and service_request.status=1)
...
--                    LEFT JOIN service_request on service_request.account_id_customer=account.id
-- LEFT JOIN service_assignment on service_assignment.service_request_id=service_request.id
                     WHERE customer.ward_id = :wards and account_service.service_id=:serviceId and account_authority.account_id_gt =:gt and account_authority.status =1  and account_service.status =1  and customer.status =1 and (account.id,account_service.service_id) NOT IN (SELECT account_id_customer,service_request.service_id FROM service_request LEFT JOIN service_assignment on service_assignment.service_request_id=service_request.id where service_assignment.servicing_status_option_id is null and service_request.status=1)
                     -- and (service_assignment.servicing_status_option_id>0 or service_assignment.servicing_status_option_id=null or service_request.id=null)  
                      ORDER BY customer.created_at DESC";
...
$non_residential =1;
     ...
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionViewNonResidentialSchedule($id)
    {
        $model = $this->findModel($id);
        $modelCustomer = new ScheduleCustomer;
        $dataProvider = new ActiveDataProvider(
        ...
        return $this->render('view-non-residential', [
            'params' => $params,
        ]);
    }
     ...
     * @return mixed
     */
    public function actionCreateNonResidentialSchedule()
    {
        $ward         = null;
        $service         = null;
        $gt         = null;
        ...
        if($ward&&$service&&$gt&&$type):
           if (isset($post['Schedule']['customer_id'])&&isset($type)&&$post['Schedule']['customer_id']!=null&&$type!=null&&sizeof($post['Schedule']['customer_id']) > 0&&$post['Schedule']['customer_id'][0]>0) {
            $modelSchedule = new Schedule;
        $ScheduleCustomer = new ScheduleCustomer();
        if ($modelSchedule->load(Yii::$app->request->post())) {
            $post = Yii::$app->request->post();
            ...
            $modelSchedule->account_id_creator= $userId;
            $modelSchedule->ward_id= $ward;
            $modelSchedule->type= $type;
            $modelSchedule->service_id= $service;
            $modelSchedule->residential_association_id= $association;
            $modelSchedule->account_id_gt= $gt;
            $modelSchedule->is_non_residential= 1;
             $modelSchedule->green_action_unit_id= $modelUser->green_action_unit_id?$modelUser->green_action_unit_id:$modelAccountGt->green_action_unit_id;
            $modelSchedule->lsgi_id= $modelUser->lsgi_id?$modelUser->lsgi_id:$modelWard->lsgi_id;
            $modelSchedule->date= $modelSchedule->date ? \Yii::$app->formatter->asDatetime($modelSchedule->date , "php:Y-m-d") : '';
            $modelSchedule->save(false);
...
$wardData = Ward::find()->where(['status'=>1])->andWhere(['id'=>$ward])->one();
$wardName = isset($wardData)?$wardData->name:'';
$modelPushMessage->message = 'Service scheduled successfully. Service is '. $serviceName. 'Ward : '.$wardName.'Total customers : '.sizeof($post['Schedule']['customer_id']);
$modelPushMessage->save(false);
$result = Yii::$app->message->sendMessage($key,$value,$modelPushMessage->message);
      ...
      {
        $modelServiceData = Service::find()->where(['id'=>$service])->andWhere(['status'=>1])->one();
                     $qry = "SELECT DISTINCT  customer.created_at,customer.lead_person_name,customer.id as customer_id,account.id as account_id,customer.ward_id as ward_id,customer.building_type_id as building_type_id,account_authority.account_id_customer as account_id_customer,building_type.name as building_type_name,customer.building_number as building_number,customer.address as address, customer.association_name as association_name, customer.association_number as association_number,customer.residential_association_id as residential_association_id FROM customer LEFT JOIN account ON account.customer_id = customer.id 
                    LEFT JOIN account_service on account_service.account_id=account.id
                    LEFT JOIN building_type ON building_type.id = customer.building_type_id LEFT JOIN account_authority ON account.id = account_authority.account_id_customer 
                     WHERE customer.ward_id = :wards and account_service.service_id=:serviceId and account_authority.account_id_gt =:gt and account_authority.status =1  and account_service.status =1  and customer.status =1 and (account.id,account_service.service_id) NOT IN (SELECT account_id_customer,service_request.service_id FROM service_request LEFT JOIN service_assignment on service_assignment.service_request_id=service_request.id where service_assignment.servicing_status_option_id is null or date(service_assignment.servicing_datetime)=:today and service_request.status=1)  
                     ";
                      if(isset($post['association'])&&$post['association']!=null){
                $qry.=" and customer.residential_association_id=:association";
                      }
                      // if($type==1){
                      //     $qry.=" and account_service.collection_interval=2";
                      //   }
                      //   elseif($type==4){
                      //     $qry.=" and account_service.collection_interval=1";
                      //   }
                      //   elseif($type==2){
                      //     $qry.=" and account_service.collection_interval=4";
                      //   }elseif($type==5){
                      //     $qry.=" and account_service.collection_interval=3";
                      //   }
                      if($type){
                          $qry.=" and account_service.collection_interval=:type";
                        }
                    $qry.=" ORDER BY customer.created_at DESC";
                  $command =  Yii::$app->db->createCommand($qry);
                  $today = date('Y-m-d');
                  $command->bindParam(':wards',$ward);
                  $command->bindParam(':serviceId',$service);
 ...
 echo Json::encode(['output' => $out, 'selected' => '']);
    ...
    public function actionRun($date=null) {
     ...
     public function actionRunPercentage($date=null) {
      Yii::$app->cron->scheduleCompletionPercentageLsgi();
    }
    public function actionRunComplaintsCount($date=null) {
      Yii::$app->cron->scheduleComplaintsCountLsgi();
    }
    public function actionRunCompletionTime($date=null) {
      Yii::$app->cron->scheduleCompletionTime();
    }
    public function actionRunComplaintResolution($date=null) {
      Yii::$app->cron->scheduleComplaintResolutionLsgi();
    }
    public function actionRunPaymentRequest($date=null) {
      Yii::$app->cron->schedulePaymentLsgi();
    }
    public function actionRunBackup($date=null) {
      Yii::$app->cron->scheduleBackup();
    }
