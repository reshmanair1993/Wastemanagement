...
class AccountGtController extends Controller
{
    ...
    public function actionIndex()
    {
      ...
      ->andWhere(['account.green_action_unit_id'=>$hks])->distinct();
...
if($hks){
    ...
    if($modelResidenceCategory&&$modelResidenceCategory->has_multiple_gt==1){
      ...
      $qry.=" NOT IN(SELECT account_id_customer FROM account_authority LEFT JOIN account on account.id=account_authority.account_id_gt WHERE account_authority.status = 1 and account_authority.account_id_gt is not null and account.green_action_unit_id=:hks) GROUP BY customer_id ORDER BY customer.created_at DESC";
    }
    else{
      $qry.="  NOT IN(SELECT account_id_customer FROM account_authority WHERE account_authority.status = 1 and account_authority.account_id_gt is not null ) GROUP BY customer_id ORDER BY customer.created_at DESC";
    }
  }
...
if($hks){
    ...
    if($modelResidenceCategory&&$modelResidenceCategory->has_multiple_gt==1){
      // $qry.=" and account_service.service_id=:service and account_service.status=1";
      $qry.=" IN(SELECT account_id_customer FROM account_authority LEFT JOIN account on account.id=account_authority.account_id_gt WHERE account_authority.status = 1 and account_authority.account_id_gt is not null and account.green_action_unit_id=:hks) GROUP BY customer_id ORDER BY customer.created_at DESC";
    }
    else{
      $qry.=" IN(SELECT account_id_customer FROM account_authority WHERE account_authority.status = 1 and account_authority.account_id_gt is not null ) GROUP BY customer_id ORDER BY customer.created_at DESC";
    }
  }
    ...
    if($modelResidenceCategory&&$modelResidenceCategory->has_multiple_gt==1){
     ...
     public function actionAssignGt()
    {
        $modelAccountGt = new AccountAuthority;
        $params = Yii::$app->request->post();
        $ok = $params && $modelAccountGt->load($params);
        $modelAccountGt->hks = isset($params['AccountAuthority']['hks'])?$params['AccountAuthority']['hks']:null;
        
        $customersList = explode(",",$params['AccountAuthority']['customer_id']);
        foreach ($customersList as $key => $value) {

            $modelGtAccount = AccountAuthority::find()
            ->leftJoin('account','account.id=account_authority.account_id_gt')
            ->where(['account_id_customer'=>$value])
            ->andWhere(['account.green_action_unit_id'=>$modelAccountGt->hks])
            ->andWhere(['account_authority.status'=>1])->one();
            if($modelGtAccount){
              $modelGtAccount->account_id_gt =$modelAccountGt->account_id_gt;
              $modelGtAccount->save(false);
               }
      }
