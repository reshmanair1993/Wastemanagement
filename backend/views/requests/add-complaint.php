    ...
    <div class="col-lg-2 col-md-4 col-sm-4 col-xs-12">
      ...
      <div class="col-lg-2 col-sm-2 col-md-8 col-xs-12">
        <label>Name</label>
           ...
           ?>
            <input type="text" name="name" value="<?php if (isset($name)) echo $name; ?>" style="background-color:#ccc;color:white;margin-top: 0px;" placeholder="Customer name..." class="form-control btn-behaviour-filter"> <a href="" class="active"></a>
    ...
    <?= GridView::widget([
      ...
      'checkboxOptions' => function($model) {
        $modelAccount = Account::find()->where(['customer_id'=>$model->id])->one();
                     return ['data-id' => $modelAccount->id,
           'class'=>'ip-chk'];
              },
    ],
               [
                  ...
                  [
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      return Customer::getFormattedCustomerId($model['id']);
                       // return Html::a($model->lead_person_name);
                  ...
                  [
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                    return isset($model['ward_id'])?Customer::getWard($model['ward_id']):null;
                 },
                  ],
                  ...
                  [
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                       $modelAccount = Account::find()->where(['customer_id'=>$model->id])->one();
                     return isset($modelAccount->id)?Customer::getLastServiceDate($modelAccount->id):null;
                    },
                  ],
                  [
                    'attribute' =>  'payment_date',
                     'label' =>'Last Payment Collected Date',
                    'contentOptions'=>[ 'style'=>'width: 250px'],
                    'format' => 'raw',
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      $modelAccount = Account::find()->where(['customer_id'=>$model->id])->one();
                     return isset($modelAccount->id)?Customer::getLastPaymentDate($modelAccount->id):null;
                    },
                  ],
