...
$height = $model->getVideoHeight($model->id);
    $width = $model->getVideoWidth($model->id);
?>
...
<?php Pjax::begin(['timeout' => 50000,'enablePushState' => false,
  ...
  <?php
      if($incidentImages){
        ...
        $modelImg = $incidentImage->fkImage;
        if($modelImg){
        $url = $modelImg->uri_full;
  ?>
<div class="col-lg-2">
<a href="<?=$modelImg->getFullUrl($url)?>" class="img-zoom">
  <img class="img-thumbnail" src="<?php echo $modelImg->getFullUrl($url);?>">
  </div>
  <?php
      }
    }
