...
$this->title = Yii::t('app', 'Update Monitoring Person', [
    'name' => $model->id,
]);
...
<div class="monitoring-group-user-update">
  ...
  $this->params['breadcrumbs'][] = ['label' => Yii::t('app', 'Monitoring Group Users'), 'url' => ['index']];
  if($model->id){
     $this->title =  'Update Monitoring Person';
  }
  else
  {
     $this->title =  'Create';
  ...
  <?php $form = ActiveForm::begin(['action' =>['update-monitoring-person','id' => $model->id],'options' => ['','data-pjax' => true,'class' => 'add-engg-form','enableAjaxValidation' => false,'enableClientValidation'=>true]]); ?>
         ...
         <div class="row">
           <div class="form-group col-lg-12 col-md-12 col-sm-6 col-12">
             ...
             echo $form->field($modelAccountWard, 'ward_id')->widget(DepDrop::classname(), [
                 'type'=>DepDrop::TYPE_SELECT2,
                 'data'=>[isset($modelAccountWard->ward_id)?$model->getWard($modelAccountWard->ward_id):''],
