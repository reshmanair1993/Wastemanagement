...
$scrollingTop = 10;
$this->title = Yii::t('app', 'Plan Enabled Customers');
...
<div class="sbhrm-asset-allocation-index">
    ...
    <div class="col-lg-2 col-md-4 col-sm-4 col-xs-12">
        <h4 class="page-title">Plan Enabled Customers</h4>
    </div>


      <div class="col-lg-3 col-sm-8 col-md-8 col-xs-12">
           <?php   $this->title =  'Plan Enabled Customers';
      ...
      <div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
        <?php $form = ActiveForm::begin(['action' =>Url::to(['customers/plan-enabled-list','set'=>1]),'options' => ['','data-pjax' => true,'class' => 'page-main-form search-form app-search hidden-sm hidden-xs m-r-10']]);?>
      ...
      if(isset($key)) { 
        ...
        $options['options'] = [$option => ['selected'=>'selected']];
            ...
            $service=Service::find()->where(['status'=>1])->andWhere(['is_package'=>1])->all();
            $listData=ArrayHelper::map($service, 'id', 'name');

            echo $form->field($modelAccountService, 'service')->dropDownList($listData, $options)->label(false)?>
            </div>
          </div>
...
?>" style="background-color:#ccc;color:white;margin-top: 0px;" placeholder="To....." class="form-control datepicker btn-behaviour-filter"> <a href="" class="active"></a>
  ...
  //                     'id' =>'pjax-customers-list', 'options'=>['data-loader'=>'.preloader']]);
   ...
   $columns = [
            [
                    ...
                    'value'=>function ($model) {
                      ...
                      return ucwords($model->fkAccount->fkCustomer->lead_person_name);
                    },
                  ],
                  [
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      return $model->fkAccount->fkCustomer->getFormattedCustomerId($model->fkAccount->fkCustomer->id);
                    },
                  ],
                   [
                    'attribute' =>  'lead_person_name',
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      return isset($model->fkAccount->fkCustomer->lead_person_phone)?$model->fkAccount->fkCustomer->lead_person_phone:'';
                    },
                  ],
                  [
                    'attribute' =>  'service',
                    ...
                    'value'=>function ($model) {
                      ...
                      return isset($model->fkPackage->name)?$model->fkPackage->name:'';
                    },
                  ],
                  [
                    'attribute' =>  'ward',
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      return $model->fkAccount->fkCustomer->fkWard->name_en;
                    },
                  ],
                  [
                    'attribute' =>  'date',
                  ...
                  [
                    ...
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      return $model->getGt($model->account_id);
                    },
                  ],
                  [
                  ...
                  [
                    'attribute' =>  'association_no',
                    'label' =>'Association Number',
                    'contentOptions'=>[ 'style'=>'width: 250px'],
                    'format' => 'raw',
                    'value'=>function ($model) {
                      $page = isset($_GET['page']) ? $_GET['page']:1;
                      return $model->fkAccount->fkCustomer->association_number;
                    },
                  ],
   ...
   $pdfHeader = [
  'L' => [
    'content' => 'Plan Enabled Customers-'.date('d-M-Y'),
  ],
  'C' => [
    'content' => '',
    'font-size' => 10,
...
$pdfFooter = [
  ...
  'line' => true,
   ...
   echo GridView::widget([
    ...
    'columns' => $columns,
    'exportConfig' => [
                   GridView::CSV => ['label' => 'Export as CSV', 'filename' => 'Plan Enabled Customers-'.date('d-M-Y')],
                   GridView::HTML => ['label' => 'Export as HTML', 'filename' => 'Plan Enabled Customers -'.date('d-M-Y')],
                   // GridView::PDF => ['label' => 'Export as PDF', 'filename' => 'Plan Enabled Customers -'.date('d-M-Y')],
                   GridView::EXCEL=> ['label' => 'Export as EXCEL', 'filename' => 'Plan Enabled Customers -'.date('d-M-Y')],
                   GridView::TEXT=> ['label' => 'Export as TEXT', 'filename' => 'Plan Enabled Customers -'.date('d-M-Y')],
                   GridView::PDF => [
    'filename' => 'Plan Enabled Customers -'.date('d-M-Y'),
    'config' => [
      'methods' => [
        'SetHeader' => [
      ...
      'options' => [
        'title' => 'Plan Enabled Customers -'.date('d-M-Y'),
        'subject' => 'Plan Enabled Customers -'.date('d-M-Y'),
