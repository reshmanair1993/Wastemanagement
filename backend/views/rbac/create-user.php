                ...
                if($wardIds){
                  foreach ($wardIds as $wardId) {
                    if($wardId){
                      ...
                      $modelWardName[] = $modelWard->id;
                    }
                  }
                }
                $wardName = isset($modelWardName)?$modelWardName:[];
                $modelAuthAssociation->ward_id = $wardName;
                echo $form->field($modelAuthAssociation, 'ward_id')->widget(Select2::classname(), [
    'data' => ArrayHelper::map(Ward::find()->where(['status'=>1])->all(),'id','name'),
    'language' => 'de',
