...
<br>
         ...
         <?php  $usersCount = Customer::find()->select('count(*)')->scalar(); 
          ...
          $users = $command->queryAll(); 
           ...
           $qry1 ="SELECT ward.name as ward, count(*) as completed FROM customer left join ward on ward.id=customer.ward_id left join account on account.customer_id=customer.id left join service_request on service_request.account_id_customer=account.id left join service_assignment on service_assignment.service_request_id=service_request.id left join service on service.id=service_request.service_id where customer.status=1 and ward.status=1 and account.status=1 and service_request.status=1 and service.status=1 and service_assignment.servicing_status_option_id>0 and service_assignment.status=1 and service.type=1";
