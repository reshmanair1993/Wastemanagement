...
use yii\widgets\ListView;
	...
	Pjax::begin(['timeout' => 50000 ,'enablePushState' => false, 'id' =>'Pjax-add-building-type','options'=>['data-loader'=>'.preloader']]);
	$form = ActiveForm::begin(['action' => ['add-building-type','id'=>$model->id],'options' => ['','data-pjax' => true,'class' => 'form-horizontal form-material','enableAjaxValidation' => false,'enableClientValidation'=>true]]);
	?>
<!-- 	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 add-tip-form" style="background: #f9f9f9; border: 1px solid #eee; padding-top: 10px; padding-bottom: 10px;">
	 <div class="col-sm-6 col-xs-6">
            <div class="form-group">
                <div class="col-sm-6 col-xs-6">
                    <?php $type= $modelBuildingType->getType();
                    $listData=ArrayHelper::map($type, 'id', 'name');
                    echo $form->field($modelBuildingType, 'id')->dropDownList($listData, ['prompt' => 'Select from the list','class'=>'form-control form-control-line'])?>
                </div>
            </div>
        </div>
	...
	<?php
		echo ListView::widget([
		    ...
		    'itemView' => function ($model, $key, $index, $widget) {
		    	return $this->render('building-type-list', [
		    		'model' => $model,
		    		'index' => $index+1,
		    	]);
		    },
