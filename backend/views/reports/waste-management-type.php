...
<div class="row bg-title">
    <div class="col-lg-2 col-md-4 col-sm-4 col-xs-12">
        <h4 class="page-title">Waste Management type Report</h4>
    </div>


      <div class="col-lg-3 col-sm-8 col-md-8 col-xs-12">
           <?php   $this->title =  'Waste Management type Report';
      ...
      <div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
        <?php $form = ActiveForm::begin(['action' => 'waste-management-type','options' => ['','data-pjax' => true,'class' => 'page-main-form search-form app-search m-r-10']]);?>
     ...
     <?php if(!isset($associations['district_id'])):?>
      ...
      $key =    Yii::$app->session->get('door');
      if(isset($key)) { 
        $option = $key;
        $options['options'] = [$option => ['selected'=>'selected']];
      }
            $listData= ['0' => 'Closed','1' => 'Open'];
      ...
      $options = ['class'=>'form-control btn-behaviour-filter','prompt' => 'Bio waste methods..','name'=>'bio_waste'];
      // $key =  isset($_POST['bio_waste'])?$_POST['bio_waste']:''; 
      $key =    Yii::$app->session->get('bio_waste');
      if(isset($key)) { 
        $option = $key;
        $options['options'] = [$option => ['selected'=>'selected']];
      }
            $method=WasteCollectionMethod::getMethodBio();
            $listData=ArrayHelper::map($method, 'id', 'name');

            echo $form->field($modelCustomer, 'bio_waste_collection_method_id')->dropDownList($listData, $options)->label(false)?>
            </div>
          </div>
           <div class="col-lg-2 col-sm-2 col-md-4 col-xs-12">
            <div class="form-group" style=" margin-top: -13px;">
      ...
      $options = ['class'=>'form-control btn-behaviour-filter','prompt' => 'Non bio waste methods..','name'=>'non_bio_waste'];
      // $key =  isset($_POST['non_bio_waste'])?$_POST['bio_waste']:''; 
      $key =    Yii::$app->session->get('non_bio_waste');
      if(isset($key)) { 
        $option = $key;
        $options['options'] = [$option => ['selected'=>'selected']];
      }
            $method=WasteCollectionMethod::getNonBioMethod();
            $listData=ArrayHelper::map($method, 'id', 'name');

            echo $form->field($modelCustomer, 'non_bio_waste_collection_method_id')->dropDownList($listData, $options)->label(false)?>
            </div>
          </div>
...
?>" style="background-color:#ccc;color:white;margin-top: 0px;" placeholder="To....." class="form-control datepicker btn-behaviour-filter"> <a href="" class="active"></a>
  ...
  $pdfHeader = [
  'L' => [
    'content' => 'Waste Management Type Report-'.date('d-M-Y'),
  ],
  'C' => [
    'content' => '',
    'font-size' => 10,
...
$pdfFooter = [
  ...
  'line' => true,
   ...
   echo GridView::widget([
    ...
    'columns' => $columns,
    'exportConfig' => [
                   GridView::CSV => ['label' => 'Export as CSV', 'filename' => 'Waste Management Type Report-'.date('d-M-Y')],
                   GridView::HTML => ['label' => 'Export as HTML', 'filename' => 'Waste Management Type Report -'.date('d-M-Y')],
                   // GridView::PDF => ['label' => 'Export as PDF', 'filename' => 'Waste Management Type Report -'.date('d-M-Y')],
                   GridView::EXCEL=> ['label' => 'Export as EXCEL', 'filename' => 'Waste Management Type Report -'.date('d-M-Y')],
                   GridView::TEXT=> ['label' => 'Export as TEXT', 'filename' => 'Waste Management Type Report -'.date('d-M-Y')],
                    GridView::PDF => [
    'filename' => 'Waste Management Type Report -'.date('d-M-Y'),
    'config' => [
      'methods' => [
        'SetHeader' => [
      ...
      'options' => [
        'title' => 'Waste Management Type Report -'.date('d-M-Y'),
