...
class ServiceRequest extends \yii\db\ActiveRecord
{
    ...
    public $name;
    public $supervisor;
    public $planned_date,$new_complaint,$residential_association_id,$customer_id;
    /**
     * {@inheritdoc}
     */
    ...
    public function rules()
    {
        return [
            ...
            [['requested_date', 'servicing_datetime', 'created_at', 'modified_at','name','supervisor','ward_id','lsgi_id','performance_point','time_of_complaint_resolution_calculated','time_of_completion_points_calculated'], 'safe'],
            [['remark'], 'string'],
...
public function behaviors() {
    ...
    public function getSpecialServiceList()
    {
        ...
        ->where(['service.is_special_service'=>1])
        ->all();
        return $service;
    }
    ...
    public function getQualityAssigned()
    {
     ...
     public function getFkServiceAssignment()
        {
                return $this->hasOne(ServiceAssignment::className(), ['service_request_id' => 'id'])->andWhere(['service_assignment.status'=>1]);
        }
    ...
    if($supervisor||$gt)
    {
      ...
      if($gt)
      {
        ...
        $query->andWhere(['>=', 'service_request.created_at',$from_date])
        ->andWhere(['<=', 'service_request.created_at',$to_date]);
      }
      if($from_date){
        $query->andWhere(['>=', 'service_request.created_at',$from_date]);
      }
      if($to_date){
        $query->andWhere(['<=', 'service_request.created_at',$to_date]);
      }
      if($status)
      {
        if($status==1)
    ...
    public function getStatuses($id=null)
    {
        ...
        $modelServiceAssignment = ServiceAssignment::find()->where(['service_request_id'=>$id])->andWhere(['status'=>1])->one();
        // print_r($modelServiceAssignment);die();
        if($modelServiceAssignment)
        {
        if($modelServiceAssignment->servicing_status_option_id==null){
            $status = 'Pending' ;
    ...
    public function getQuantityCollected($keyword=null,$service=null,$gt=null,$from=null,$to=null,$type=null,$status=null,$ward=null,$waste_type=null)
    {
        $count = 0;
        $supervisor = null;
        $unit = null;
