...
class EvaluationConfigComplaintsCount extends \yii\db\ActiveRecord
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'start_value_count' => Yii::t('app', 'Start Value Percentage'),
            'end_value_count' => Yii::t('app', 'End Value Percentage'),
    ...
    {
        $err  = EvaluationConfigComplaintsCount::find()->where(['>=','end_value_count',$this->start_value_count])->andWhere(['<=','start_value_count',$this->start_value_count])->andWhere(['status'=>1])->one();
        if($err)
