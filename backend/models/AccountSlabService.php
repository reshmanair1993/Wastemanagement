...
class AccountSlabService extends \yii\db\ActiveRecord
{
   ...
   public function getServices()
    ...
    {
        $err  = AccountSlabService::find()->where(['service_id'=>$this->service_id])->andWhere(['account_id_customer'=>$this->account_id_customer])->andWhere(['status'=>1])->one();
        if($err)
        $this->addError($attribute,'Servive is already taken ');
    }
}
