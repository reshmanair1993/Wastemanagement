...
class Customer extends \yii\db\ActiveRecord
{
     ...
     * @var mixed
     */
    public $district_id, $block_id, $assembly_constituency_id, $lsgi_id,$code,$no_association,$building_type_name,$agency,$qr_code,$qr_code_value,$package_id;
    /**
     * {@inheritdoc}
     */
    ...
    public function rules()
    {
        return [
            ...
            [['address'], 'string'],
            // [['lat', 'lng'], 'number'],
            [['created_at', 'modified_at','image_id','building_sub_type','lat', 'lng','customer_id','service_secret_otp','qr_code','package_id'], 'safe'],
            ...
            [['qr_code_value'],'if_qrcode_assigned'],
        ];
    }
    public function if_qrcode_assigned($attribute){
   $qr_code = $this->qr_code_value;
   $modelQrCode = QrCode::find()->where(['value'=>$qr_code])->andWhere(['is not','customer_id',null])->andWhere(['status'=>1])->one();
   if($modelQrCode){
     $this->addError('qr_code_value','Qr code is not valid');
     return;
   }else
   {
    $modelQrCode = QrCode::find()->where(['value'=>$qr_code])->andWhere(['status'=>1])->one();
    if(!$modelQrCode){
     $this->addError('qr_code_value','Qr code is not valid');
     return;
   }
...
public function behaviors() {
   ...
   /**
    ...
    {
        return $this->hasOne(QrCode::className(), ['id' => 'qr_code_id']);
    }
    ...
    public function deleteCustomer($id)
    {
        ...
        if($modelCustomer)
        {
            ...
            if($modelAccount)
            {
                $modelAccount->status= 0;
                $modelAccount->save(false);
            }
            $modelWardCustomerCount = WardCustomerCount::find()->where(['ward_id'=>$modelCustomer->ward_id])->andWhere(['ward_customer_count.date'=>date($modelCustomer->created_at)])->one();
   ...
   public function getFkImage()
  {
   ...
   public function getHouseCount($id = null,$from=null,$to=null,$ward=null)
    {     
           $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=1";
         if($from!=null)
        {
...
else
{
    ...
    public function getFlatCount($id = null,$from=null,$to=null,$ward=null)
    {
       
      $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=3";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    public function getShopCount($id = null,$from=null,$to=null,$ward=null)
    {
     $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=2";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    $count =0;
     ...
     public function getHospitalCount($id = null,$from=null,$to=null,$ward=null)
    {
      
      $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=5";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    public function getPublicPlaceCount($id = null,$from=null,$to=null,$ward=null)
    {
      
     $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=6";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    $count =0;
     ...
     public function getReligiousCount($id = null,$from=null,$to=null,$ward=null)
    {
    $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=11";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    public function getOfficeCount($id = null,$from=null,$to=null,$ward=null)
    {
      $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=7";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    public function getAuditoriumCount($id = null,$from=null,$to=null,$ward=null)
    {
      
     $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=9";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    $count =0;
     ...
     public function getMarketCount($id = null,$from=null,$to=null,$ward=null)
    {
     
      $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.creator_account_id=:id and  customer.building_type_id=10";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
        $count =0 ;
        // $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=1 and customer.status=1 and customer.is_public_customer=0" ;
        $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.ward_id=:id and  customer.building_type_id=1";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
       $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=3 and customer.status=1 and customer.is_public_customer=0" ;
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    public function getWardShopCount($id = null,$from=null,$to=null)
    {
       ...
       $count =0 ;
        // $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=2 and customer.status=1 and customer.is_public_customer=0" ;
       $query = "SELECT count(customer.id)  as count FROM `customer` WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.ward_id=:id and  customer.building_type_id=2";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
     ...
     public function getWardHospitalCount($id = null,$from=null,$to=null)
    {
       ...
       $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=5 and customer.status=1 and customer.is_public_customer=0" ;
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    public function getWardPublicPlaceCount($id = null,$from=null,$to=null)
    {
      ...
      $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=6 and customer.status=1 and customer.is_public_customer=0" ;
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
       $count =0 ;
        // $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=11 and customer.status=1 and customer.is_public_customer=0" ;
       $query = "SELECT count(customer.id)  as count FROM `customer`  WHERE (`customer`.`status`=1) AND (`customer`.`is_public_customer`=0) and customer.ward_id=:id and  customer.building_type_id=11";
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
       $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=7 and customer.status=1 and customer.is_public_customer=0" ;
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
       $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=9 and customer.status=1 and customer.is_public_customer=0" ;
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
       $count =0 ;
        $query = "SELECT count(customer.id)  as count FROM customer WHERE customer.ward_id=:id and customer.building_type_id=10 and customer.status=1 and customer.is_public_customer=0" ;
         if($from!=null)
        {
            $query.= " and customer.created_at >= :from";
        }
...
else
{
    ...
    {
        $name  = null;
        $service = Service::find()->where(['status' => 1])->andWhere(['id' => $service])->one();
        if ($service)
        {
            $name = $service->name;
        }
    ...
    public function getResidentailAssociation($id)
    {
     ...
     public function getLastServiceDate($id = null)
    {
        $date ='';
        $service = ServiceAssignment::find()
        ->leftjoin('service_request','service_request.id=service_assignment.service_request_id')
        ->where(['is not','service_assignment.servicing_datetime',null])
        ->andWhere(['service_request.status' => 1])
        ->andWhere(['service_request.account_id_customer' => $id])
        ->orderby('service_assignment.servicing_datetime DESC')
        ->one();
        if ($service)
        {
            $date = $service->servicing_datetime;
        }

        return $date;
    }
     public function getLastPaymentDate($id = null)
    {
        $date ='';
        $payment = Payment::find()
        ->leftjoin('payment_request','payment_request.id=payment.payment_request_id')
        // ->where(['is not','payment_request.servicing_datetime',null])
        ->where(['payment_request.status' => 1])
        ->andWhere(['payment_request.account_id_customer' => $id])
