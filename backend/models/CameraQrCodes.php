...
/**
 * This is the model class for table "camera_qr_code".
 *
 * @property int $id
 ...
 */
class CameraQrCodes extends \yii\db\ActiveRecord
{
    ...
    public static function getAllQuerys() {
     $query = static::find()->where(['camera_qr_code.status'=>1])
                            ;
     return $query;
   }
    public static function getAllQuery($start=null,$limit=null,$lsgi_id=null) {
     $query = static::find()->where(['camera_qr_code.status'=>1])
                            ->andWhere(['camera_qr_code.account_id'=>null])
                            ->andWhere(['=','camera_qr_code.lsgi_id',$lsgi_id]);
     if($start)
     {
