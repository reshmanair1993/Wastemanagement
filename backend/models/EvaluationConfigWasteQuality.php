...
class EvaluationConfigWasteQuality extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['quality_type_id','performance_point'], 'required'],
            [['lsgi_id', 'quality_type_id', 'status'], 'integer'],
            [['performance_point'], 'number'],
              [['quality_type_id'],'new_and_unique'],
    ...
    {
        $err  = EvaluationConfigWasteQuality::find()->where(['quality_type_id'=>$this->quality_type_id])->andWhere(['status'=>1])->one();
        if($err)
        $this->addError($attribute,'Quality is already taken ');
    }
}
